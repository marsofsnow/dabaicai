...
namespace otcsystem
{
    ...
    static constexpr name sysacnt_account{"sysaccount"_n};
    ...
    struct [[eosio::table("sysconfig"), eosio::contract("otcsystem")]] sys_config
    {
        ...
        EOSLIB_SERIALIZE(sys_config,
                         (signin_expiration)(rebate_max_level)(default_percent_precision)(default_rebate_rate)(default_referral_rate)(default_price_limit)(default_fee_rate))
    };
    ...
    struct [[eosio::table, eosio::contract("otcsystem")]] apply_t
    {
        ...
        EOSLIB_SERIALIZE(apply_t, (owner)(rebate_rate)(team_users)(state)(request)(reply)(apply_time)(approve_time))
    };
    ...
    typedef std::vector<asset> vtrfeerate;
    ...
    private:
        ...
        void check_nickname(const string &nickname);
        ...
        void add_referrals(const name &inviter);
        void update_referrals_out(const name &proposer);
        void update_referrals_in(const name &owner, const name &inviter);
        void update_user_rate(const name &owner, uint32_t referral_rate);
        void update_team_rate(const name &owner, const name &inviter);
        void update_user_admin(const name &owner, const name &admin);
    ...
    public:
        ...
        // 返佣
        [[eosio::action]] void apply(const name &owner, double rebate_rate, uint32_t team_users, const string &comment);
